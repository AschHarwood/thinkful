...
with con:
    ...
    cur.execute("SELECT city, state FROM weather LEFT OUTER JOIN cities ON city = name WHERE warm_month IS 'July'")

    rows = cur.fetchall() #grabbing all the rows
    cols = [desc[0] for desc in cur.description]
    df = pd.DataFrame(rows, columns=cols)

    dftotuple = df[['city', 'state']]
    dfvalues = [tuple(x) for x in dftotuple.values]
    warmcitieslist = [', '.join(item) for item in dfvalues]
